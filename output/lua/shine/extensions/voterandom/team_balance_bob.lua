...
--local inspect = Shine.LoadPluginFile( PluginName, "inspect.lua" )
local bobShuffle = Shine.LoadPluginFile( PluginName, "bobshuffle.lua" )
...
function BalanceModule:OptimiseTeams( TeamMembers, RankFunc, TeamSkills )
	...
	if self:ShouldOptimiseHappiness( TeamMembers ) then
		...
		self:SaveHappiness( TeamMembers )
	end
end
...
BalanceModule.ShufflingModes = {
	...
	[ Plugin.ShuffleMode.HIVE ] = function( self, Gamerules, Targets, TeamMembers )
		...
		if EnforceTeamsizes then
			maxPlayersTeam = EnforceTeamsizes.GetMaxTeamPlayers()
		else
		...
		if sampledataFile ~= nil then
			local text = bobShuffle.showTeams(t0, t1, t2)
		...
		TeamMembers[1] = {}
		TeamMembers[2] = {}
		for _, Player in ipairs(t1) do
			table.insert(TeamMembers[1], Player.player)
		end
