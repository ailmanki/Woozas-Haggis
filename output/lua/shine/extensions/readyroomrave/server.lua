...
function Plugin:OnConsoleCreateSpray(client)
    ...
    -- spam protection
    if (Shared.GetTime() - self.commandTime < 1.4) then return end
    ...
    if trace.fraction ~= 1 then
        ...
        local myangle = math.rad(90)
        local cx = coords.xAxis
        local cos = math.cos(myangle)
        local sin = math.sin(myangle)
        coords.xAxis = (cx * cos) + (coords.zAxis * -sin);
        coords.zAxis = (cx * sin) + (coords.zAxis * cos);
